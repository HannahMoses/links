...
import sqlite3 #a lightweight database
...
links = [
    ...
    # links is a list of Link objects. Links have a handful of properties. For example,
    # a Link's number of votes can be accessed by link.votes if "link" is a Link.
...
def query():
    ...
    for l in links:
        if l.submitter_id == 62443:
            ...
            return submissions
            #db is an in-memory database that can respond to SQL queries using execute()function
    # db is a global variable ;it represents our sqlite database
    #dbis an in-memory database that can respond to SQL queries using execute()function
    # For example, if we run
    # c = db.execute("select * from links")
            # c will be a cursor to the results of that query.We can use the fetchmany()
            # function on the cursor to convert that cursor into a list of results.These
            # results  WON'T BE LINKS; they'll be tuples, but they can be passed after they
            # are turned into a link.


            #to create a table named 'links' in Python file,using sqlite,and populate it:
# db = sqlite3.connect(':memory:')
# db.execute('create table links' +
#            '(id integer, submitter_id integer,submotted_time integer,'+
#            'votes integer,title text,url text)' )
# for l in links:
#     db.execute('insert into links values(?,?,?,?,?,?)',l)
        #to print all the votes for all of the links, do this:
# c = db.execute("select * from links")
# for link_tuple in c:
...
#     print link.votes
    ...
    #To run some sql in .py file.To get some sql data from the table links,this code
    #creates a cursor=a position in the sql database
# def query():
#     c = db.execute("select * from links")
#     results = c.fetchall()#calling function on the cursor c to load data out of database
#                          #into this list named,results
#     return results
#
        #let's look at just the titles from our database table links
def query():
...
# class MainHandler(webapp2.RequestHandler):
#     def get(self):
#         self.response.write('Hello world of '+" "+Myname+ " !")
#
# app = webapp2.WSGIApplication([
#     ('/', MainHandler)
# ], debug=True)
